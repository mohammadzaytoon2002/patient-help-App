...
class PredictionPageresult extends StatefulWidget {
  @override
  _PredictionPageState createState() => _PredictionPageState();
}

class _PredictionPageState extends State<PredictionPageresult> {
  String token = "";
  List dfuPredictions = [];
  ...
  _fetchPredictions() async {
    try {
      var dfuResponse = await http.get(
        Uri.parse('http://192.168.1.107:8000/api/getpredict_dfu'),
        headers: {
      ...
      var diabetesResponse = await http.get(
        Uri.parse('http://192.168.1.107:8000/api/getpredictdiabetes'),
        headers: {
          'Authorization': 'Bearer $token',
        },
      );
      ...
      var genomicResponse = await http.get(
        Uri.parse('http://192.168.1.107:8000/api/getpredictgenomic'),
        headers: {
          'Authorization': 'Bearer $token',
        },
      );
      ...
      var retinaResponse = await http.get(
        Uri.parse('http://192.168.1.107:8000/api/getpredict_retina'),
        headers: {
          'Authorization': 'Bearer $token',
        },
      );
      ...
      children: [
        ...
        predictions.isEmpty
            ...
            : Column(
                children: predictions.map((prediction) {
                  String imageUrl =
                      'http://192.168.1.107:8000${prediction['image']}';
                  return Container(
                    margin: EdgeInsets.symmetric(vertical: 8),
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(15),
